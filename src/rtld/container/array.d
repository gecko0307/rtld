...
struct Array(T, size_t chunkSize = 32)
{
   private:
     ...
     */
    private T* storage() nothrow @nogc
    {
        if (numChunks == 0)
    ...
    private const(T)* readOnlyStorage() const nothrow @nogc
    {
        if (numChunks == 0)
            return staticStorage.ptr;
        else
  ...
  public:
     ...
     */
    @property bool isDynamic() const nothrow @nogc
    {
     ...
     */
    T opIndex(const(size_t) index) nothrow @nogc
    {
        return data[index];
    }
     ...
     * Access by slice.
     */
    T[] opSlice(const(size_t) start, const(size_t) end) nothrow @nogc
    {
     ...
     */
    T opIndexAssign(T t, const(size_t) index) nothrow @nogc
    {
        data[index] = t;
        return t;
    }
