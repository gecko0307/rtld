...
class Arena: Owner
{
    ...
    {
        CleanupNode* next;
        void function(void* obj) destructor;
        void* object;
    }

    CleanupNode* cleanupHead = null;
    ...
    {
        // Call destructors
        CleanupNode* current = cleanupHead;
        while (current !is null)
        {
            current.destructor(current.object);
     ...
     */
    T create(T, A...)(A args, string file = __FILE__, int line = __LINE__)
        if (is(T == class))
    {
        enum objectSize = __traits(classInstanceSize, T);
        
        ubyte* memory = this.allocate(objectSize, T.alignof).ptr;
        if (memory is null)
        ...
        {
            CleanupNode* node = cast(CleanupNode*)this.allocate(CleanupNode.sizeof, CleanupNode.alignof).ptr;
            if (node is null)
                outOfMemoryError();
            
            node.object = cast(void*)instance;
            node.destructor = function(void* p) {
                (cast(T*)p).__dtor();
            };
            node.next = cleanupHead;
            cleanupHead = node;
        }
     ...
     */
    T create(T)(size_t length, string file = __FILE__, int line = __LINE__)
        if (isDynamicArray!T)
    {
        alias AT = ElementType!T;
        size_t objectSize = length * AT.sizeof;
        
        ubyte[] mem = allocate(objectSize, AT.alignof);
        if (mem.length == 0)
            outOfMemoryError(file, line);
        
        T arr = cast(T)mem[0..objectSize];
        foreach(ref v; arr)
            v = v.init;
        return arr;
    }
