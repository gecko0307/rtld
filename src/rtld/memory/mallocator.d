...
class Mallocator: Allocator
{
    ...
    {
        return cast(uint)max2(double.alignof, real.alignof);
    }
    ...
    static @property Mallocator instance() @nogc nothrow
    {
        ...
        if (instance_ is null)
        {
            ...
            if (memory is null)
                return null;
            const(void)[] initSymbol = __traits(initSymbol, Mallocator);
            memcpy(memory, initSymbol.ptr, size);
            instance_ = cast(Mallocator)memory;
        }
        return instance_;
    }
